...
export default function EditAthleteDetails(props: AthleteDetailsProps) {
    ...
    const checkFieldsChanged = () => {
        ...
        if (user.data.summary != getViewStateFieldValue(viewState, "summary")) {
            return true;
        }
        return false;
    }
                ...
                try {
                    await api.members.updateMemberDetails(MemberIdMeParameter.TypeMe, {
                        ...
                        data: {
                            ...
                            summary: viewState.fields.summary.value || null
                        }
                    });
        ...
        <RenderViewState state={viewState}>
            <div className="pt-4">
                ...
                <div className="flex flex-col gap-x-4 lg:flex-row items-center justify-center w-2/3 mx-auto">
                    <SelectField required={true} items={Object.keys(HeightUnit)} hideDefault={true} label="Unit" stateKey="heightUnit" className="py-2" value={getViewStateFieldValue(viewState, "heightUnit")} onChange={handleInputChange} />
                    {getViewStateFieldValue(viewState, "heightUnit") === HeightUnit.Meters
                        ? <InputField required={true} stateKey="heightMetersValue" placeholder="ex. 1.95 m" className="py-2 lg:w-2/3" maxLength={4} label="Meters" value={getViewStateFieldValue(viewState, "heightMetersValue")} onChange={handleInputChange} />
                        : <>
                            <InputField required={true} stateKey="heightFeetValue" placeholder="ex. 5'" maxLength={1} className="py-2 lg:w-1/3" label="Feet" value={getViewStateFieldValue(viewState, "heightFeetValue")} onChange={handleInputChange} />
                            <InputField required={true} stateKey="heightInchesValue" placeholder="ex. 7 in" maxLength={2} className="py-2 lg:w-1/3" label="Inches" value={getViewStateFieldValue(viewState, "heightInchesValue")} onChange={handleInputChange} />
                        </>
                    }
                </div>
            ...
            <div className="pt-4">
                Weight
                <div className="flex flex-col gap-x-4 lg:flex-row items-center justify-center w-2/3 mx-auto">
                    <SelectField required={true} items={Object.keys(WeightUnit)} hideDefault={true} label="Unit" stateKey="weightUnit" className="py-2" value={getViewStateFieldValue(viewState, "weightUnit")} onChange={handleInputChange} />
                    {getViewStateFieldValue(viewState, "weightUnit") === WeightUnit.Pounds
                        ? <InputField required={true} stateKey="weightPoundsValue" placeholder="ex. 150 pounds" className="py-2 lg:w-2/3" maxLength={4} label="Pounds" value={getViewStateFieldValue(viewState, "weightPoundsValue")} onChange={handleInputChange} />
                        : <InputField required={true} stateKey="weightKgValue" placeholder="ex. 80kg" className="py-2 lg:w-2/3" maxLength={4} label="Kilograms" value={getViewStateFieldValue(viewState, "weightKgValue")} onChange={handleInputChange} />
                    }
                </div>
            ...
            <div className="flex flex-col gap-x-4 lg:flex-row items-center justify-center mx-auto">
                <TextArea wordLimit={500} stateKey="summary" placeholder="Your Athletic Summary" label="Summary" value={getViewStateFieldValue(viewState, "summary")} onChange={handleInputChange}/>
            </div>
