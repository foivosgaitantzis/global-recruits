...
export function getOnboardingPage(userData: UserModel): (number | undefined) {
    if (userData.type === "athlete") {
        ...
        if (data.data.firstName || data.data.lastName || data.data.dateOfBirth || data.data.country || data.data.city) {
            if (data.data.firstName && data.data.lastName && data.data.dateOfBirth && data.data.country && data.data.city) {
                if (data.data.height && data.data.height.value && data.data.height.unit && data.data.weight && data.data.weight.value && data.data.weight.unit) {
                    return 3;
                }
                return 2;
            }
